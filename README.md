# Islamic-Prayer-Times
For Nintendo Switch

![background](https://i.postimg.cc/j5ZPBCHB/Icon.png)

Works in Applet mode.

The app itself is a very basic shell, and is lightweight. I made this because the Nintendo Switch is such a capable device, it has the opportunity
to be the ultimate all-in-one device, however I was surprised an app like this doesn't exist, so I made it in case you don't have a phone or PC on you but need to know what time a prayer is.
There is a list of 114 major cities in the app, and You can also search for any city you want outside that list as long as the app can find it and geolocate.

# Installation

To install, go to the Switch folder on your sd card, create a folder called "prayer-times", and put the NRO File in it. That's it.

Uses AlAdhan api and Islamic Society of North America method.

Planned updates: 
- Working offline for major cities calculating by latitude/longitude (highest priority right now)
- Being able to be added to Tesla/ultrahand menu
- A real nice looking more functional GUI
- Potentially a sys-module that can give you notifications?

![background](https://i.postimg.cc/tTt2JXhr/image.png)
Installed on home screen with NSP converter, icon changed with SwitchU

If you have any suggestions let me know on GBATemp. Thanks and enjoy
![NS](https://img.shields.io/badge/-Nintendo%20Switch-e4000f?style=flat&logo=Nintendo%20Switch)
